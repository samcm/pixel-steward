...
func tools() []map[string]any {
	return []map[string]any{
		...
		{"name": "studio_watch", "description": "Continuously sample a workspace image for live operator preview and history without using inference per frame. This never changes the physical display; call studio_publish explicitly when a finished image is ready.", "inputSchema": objectSchema(map[string]any{"path": map[string]string{"type": "string"}, "fps": map[string]any{"type": "number", "minimum": 0.01}}, []string{"path", "fps"})},
